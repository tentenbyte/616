...
(function(global) {
     ...
     */
    SimpleColumnarDB.prototype.initializePersistence = function() {
        var self = this;
        
        // 创建IndexedDB管理器 - 使用固定数据库名
        this.indexedDBManager = new global.IndexedDBManager('ColumnarTableDB', 1);
        
        // 脏数据跟踪
        this.dirtyColumns = new Set();
        this.dirtyMetadata = false;
        this.dirtyDisplayState = false;
        this.lastSaveTime = 0;
        
        return this.indexedDBManager.initialize().then(function() {
            console.log('✅ 列式数据库持久化系统初始化完成');
            return self.loadFromIndexedDB();
        });
    };
     ...
     */
    SimpleColumnarDB.prototype.saveToIndexedDB = function() {
        if (!this.indexedDBManager || !this.indexedDBManager.isReady) {
            return Promise.reject(new Error('持久化系统未初始化'));
        }

        var self = this;
        var startTime = performance.now();
        
        console.log('💾 开始保存列式数据到IndexedDB...');
        
        var promises = [];

        // 1. 保存元数据（如果有变更）
        if (this.dirtyMetadata) {
            promises.push(this.indexedDBManager.saveMetadata({
                maxRows: this.maxRows,
                maxCols: this.maxCols,
        ...
        // 2. 保存脏列数据和字符串池
        for (var col = 0; col < this.maxCols; col++) {
            if (this.dirtyColumns.has(col)) {
                // 保存列数据
                promises.push(this.indexedDBManager.saveColumnData(col, this.columns[col]));
                
                // 保存字符串池
                promises.push(this.indexedDBManager.saveStringPool(
                    col, 
                    this.stringPools[col],
                    this.stringMaps[col]
                ));
            }
        }
        ...
        // 3. 保存显示状态（如果有变更）
        if (this.dirtyDisplayState) {
            promises.push(this.indexedDBManager.saveDisplayState(this.displayIndices, {
                lastSortColumn: this.lastSortColumn,
                lastSortAscending: this.lastSortAscending
            }));
        }
        ...
        return Promise.all(promises).then(function() {
            ...
            // 清除脏标记
            self.dirtyColumns.clear();
            self.dirtyMetadata = false;
            self.dirtyDisplayState = false;
            
            console.log('✅ 列式数据保存完成 (' + duration.toFixed(2) + 'ms, ' + promises.length + '个操作)');
            return {
                duration: duration,
                operations: promises.length,
                timestamp: self.lastSaveTime
            };
        });
    };
     ...
     */
    SimpleColumnarDB.prototype.loadFromIndexedDB = function() {
        if (!this.indexedDBManager || !this.indexedDBManager.isReady) {
            return Promise.reject(new Error('持久化系统未初始化'));
        }
        ...
        console.log('📂 开始从IndexedDB加载列式数据...');

        // 1. 加载元数据
        return this.indexedDBManager.loadMetadata().then(function(metadata) {
            if (!metadata) {
                console.log('ℹ️ 未找到已保存的数据，使用空表格');
                return { loaded: false, reason: 'no_data' };
            }

            console.log('📋 找到已保存的元数据:', metadata);

            // 恢复基本信息
            self.totalRows = metadata.totalRows;
            self.currentRows = metadata.totalRows;  // 确保currentRows也被设置
            self.visibleRows = metadata.visibleRows;
            if (metadata.columnNames) {
                self.columnNames = metadata.columnNames;
            }
            ...
            for (var col = 0; col < self.maxCols; col++) {
                loadPromises.push(self._loadColumn(col));
            }

            return Promise.all(loadPromises).then(function(results) {
                // 3. 加载显示状态
                return self.indexedDBManager.loadDisplayState().then(function(displayState) {
                    if (displayState) {
                        // 恢复显示索引
                        for (var i = 0; i < displayState.displayIndices.length && i < self.maxRows; i++) {
                            self.displayIndices[i] = displayState.displayIndices[i];
                        }
                        
                        // 恢复排序状态
                        if (displayState.sortState) {
                            self.lastSortColumn = displayState.sortState.lastSortColumn;
                            self.lastSortAscending = displayState.sortState.lastSortAscending;
                        }
                    }

                    var duration = performance.now() - startTime;
                    var loadedColumns = results.filter(function(r) { return r.loaded; }).length;
                    
                    console.log('✅ 列式数据加载完成 (' + duration.toFixed(2) + 'ms, ' + 
                               loadedColumns + '列, ' + self.totalRows + '行)');
     ...
     * @returns {Promise} 加载结果
     */
    SimpleColumnarDB.prototype._loadColumn = function(col) {
        var self = this;
        
        return Promise.all([
            this.indexedDBManager.loadColumnData(col),
            this.indexedDBManager.loadStringPool(col)
        ]).then(function(results) {
            var columnData = results[0];
            var stringPoolData = results[1];
            
            var loaded = false;
            
            // 恢复列数据
            if (columnData) {
                for (var row = 0; row < columnData.length && row < self.maxRows; row++) {
                    self.columns[col][row] = columnData[row];
                }
                loaded = true;
            }
            ...
            // 恢复字符串池
            if (stringPoolData) {
                self.stringPools[col] = stringPoolData.stringPool;
                self.stringMaps[col] = stringPoolData.stringMap;
                self.nextStringIds[col] = stringPoolData.stringPool.length;
                loaded = true;
            }
            
            return { columnIndex: col, loaded: loaded };
        });
    };
     ...
     * @returns {Promise} 保存结果
     */
    SimpleColumnarDB.prototype.saveIncrementally = function() {
        if (this.dirtyColumns.size === 0 && !this.dirtyMetadata && !this.dirtyDisplayState) {
            console.log('ℹ️ 无脏数据，跳过保存');
     ...
     * @param {number} col 列索引
     */
    SimpleColumnarDB.prototype.markColumnDirty = function(col) {
     ...
     * 标记元数据为脏数据
     */
    SimpleColumnarDB.prototype.markMetadataDirty = function() {
     ...
     * 标记显示状态为脏数据
     */
    SimpleColumnarDB.prototype.markDisplayStateDirty = function() {
        this.dirtyDisplayState = true;
    };
     ...
     */
    SimpleColumnarDB.prototype.clearPersistentData = function() {
        if (!this.indexedDBManager) {
            return Promise.reject(new Error('持久化系统未初始化'));
        }
        
        return this.indexedDBManager.clearAllData();
    };
     ...
     * @returns {Object} 统计信息
     */
    SimpleColumnarDB.prototype.getPersistenceStats = function() {
        if (!this.indexedDBManager) {
            return { error: '持久化系统未初始化' };
        }
        
        var stats = this.indexedDBManager.getStats();
        return {
            indexedDB: stats,
            dirtyColumns: this.dirtyColumns.size,
            dirtyMetadata: this.dirtyMetadata,
            dirtyDisplayState: this.dirtyDisplayState,
            lastSaveTime: this.lastSaveTime,
            isPersistenceReady: this.indexedDBManager.isReady
        };
    };
    ...
    // 重写setValue方法，添加脏数据跟踪
    var originalSetValue = SimpleColumnarDB.prototype.setValue;
    SimpleColumnarDB.prototype.setValue = function(row, col, value) {
        var result = originalSetValue.call(this, row, col, value);
        this.markColumnDirty(col);
        return result;
    };

    // 重写sortByColumn方法，添加显示状态跟踪
    var originalSortByColumn = SimpleColumnarDB.prototype.sortByColumn;
    SimpleColumnarDB.prototype.sortByColumn = function(col, ascending) {
        var result = originalSortByColumn.call(this, col, ascending);
        this.markDisplayStateDirty();
        return result;
    };

    // 重写filterByValues方法，添加显示状态跟踪
    var originalFilterByValues = SimpleColumnarDB.prototype.filterByValues;
    SimpleColumnarDB.prototype.filterByValues = function(col, selectedValues) {
        var result = originalFilterByValues.call(this, col, selectedValues);
        this.markDisplayStateDirty();
        return result;
    };
