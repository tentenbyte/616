...
(function(global) {
     ...
     * 触发自动保存
     */
    SimpleTableCore.prototype.triggerAutoSave = function() {
        var self = this;
     ...
     * 执行自动保存
     */
    SimpleTableCore.prototype.performAutoSave = function() {
        if (!this.db || !this.db.indexedDBManager) {
            console.log('⚠️ 持久化系统未初始化，跳过自动保存');
            return;
        }
        ...
        console.log('💾 触发智能自动保存...');
        this.db.saveIncrementally()
            .then(function(result) {
                if (!result.skipped) {
                    self.lastSaveTime = now;
                    console.log('✅ 自动保存完成 (' + result.duration.toFixed(2) + 'ms, ' + 
                               result.operations + '个操作)');
                    
                    // 🔍 调试：显示保存的脏数据信息
                    var stats = self.db.getPersistenceStats();
                    console.log('🔍 保存统计:', {
                        dirtyColumns: stats.dirtyColumns,
                        dirtyMetadata: stats.dirtyMetadata,
                        timestamp: new Date(result.timestamp).toLocaleTimeString()
                    });
                    
                    // 更新状态提示
                    if (typeof updateStatus === 'function') {
                        updateStatus('数据已自动保存到IndexedDB', 'success');
                    }
                } else {
                    console.log('ℹ️ 无脏数据，跳过自动保存');
                }
            })
            .catch(function(error) {
                console.error('❌ 自动保存失败:', error);
                if (typeof updateStatus === 'function') {
                    updateStatus('自动保存失败: ' + error.message, 'error');
                }
            });
    };
     ...
     * 手动保存
     */
    SimpleTableCore.prototype.save = function() {
        if (!this.db || !this.db.indexedDBManager) {
        ...
        return this.db.saveToIndexedDB().then(function(result) {
            self.lastSaveTime = Date.now();
            console.log('✅ 手动保存完成:', result);
            
            if (typeof updateStatus === 'function') {
                updateStatus('数据手动保存完成', 'success');
            }
