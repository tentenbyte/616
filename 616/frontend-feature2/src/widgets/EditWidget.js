...
(function(global) {
    ...
    EditWidget.prototype.setupCanvasClickHandler = function() {
        ...
        this.canvas.addEventListener('click', function(e) {
            ...
            try {
                ...
                // 计算实际数据中的行列位置（加上滚动偏移）
                var actualCol = col + Math.floor(scrollX / tableConfig.cellWidth);
                var actualRow = row + Math.floor(scrollY / tableConfig.cellHeight);
                ...
                // 使用实际数据位置进行后续处理
                self.state.currentCell = { row: actualRow, col: actualCol };
                
                // 通知TableCore选择变化
                if (self.tableCore && self.tableCore.selectCell) {
                    self.tableCore.selectCell(actualRow, actualCol);
                }
                
                // 检查该单元格是否需要滚动调整
                var needsScroll = self.ensureCellVisible(actualRow, actualCol);
                
                // 如果需要滚动，等待渲染完成后再移动input
                if (needsScroll) {
                    setTimeout(function() {
                        self.moveToCell(actualRow, actualCol);
                        self.focusInput(false);
                        self.showHelperForColumn(actualCol);
                    }, 50);
                } else {
                    self.moveToCell(actualRow, actualCol);
                    self.focusInput(false);
    ...
    EditWidget.prototype.forceTableRender = function() {
        try {
            // 方法1: 使用非节流的原始渲染方法
            if (this.tableCore && this.tableCore.tableWidget && this.tableCore.tableWidget.renderer && this.tableCore.tableWidget.renderer._originalRender) {
                this.tableCore.tableWidget.renderer._originalRender(this.tableCore.db, this.tableCore.state ? this.tableCore.state.selectedCell : null);
            }
            // 方法2: 使用TableCore的render方法
            else if (this.tableCore && this.tableCore.render) {
                this.tableCore.render();
            }
            // 方法3: 使用tableWidget的render方法
            else if (this.tableCore && this.tableCore.tableWidget && this.tableCore.tableWidget.render) {
                this.tableCore.tableWidget.render();
            }
    ...
    EditWidget.prototype.ensureCellVisible = function(row, col) {
        if (!this.tableCore || !this.tableCore.renderer) {
            return false;
        }
        ...
        if (newScrollX !== scrollX || newScrollY !== scrollY) {
            ...
            if (this.tableCore && this.tableCore.render) {
                ...
                requestAnimationFrame(function() {
                    if (self.tableCore.renderer && self.tableCore.renderer.ctx) {
                        var ctx = self.tableCore.renderer.ctx;
                        var canvas = self.tableCore.renderer.canvas;
