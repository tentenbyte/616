...
(function(global) {
    ...
    function TableRenderer(canvas, config, dependencies) {
        ...
        this.scrollbarWidget = dependencies.scrollbarWidget; // 滚动条组件引用
        ...
        // 绑定方法 - 保存原始render方法用于立即渲染
        this._originalRender = this.render.bind(this);
        this.render = global.Helpers.throttle(this._originalRender, 16); // 60fps
    ...
    TableRenderer.prototype.updateViewport = function(tableData) {
        ...
        // 计算可见的行列范围（支持平滑滚动，扩展渲染范围）
        this.viewport.startRow = Math.max(0, Math.floor(this.scrollY / this.config.cellHeight) - 1);
        this.viewport.endRow = Math.min(
            maxRows - 1,
            Math.ceil((this.scrollY + canvasHeight - this.config.headerHeight) / this.config.cellHeight) + 1
        );
        
        this.viewport.startCol = Math.max(0, Math.floor(this.scrollX / this.config.cellWidth) - 1);
        this.viewport.endCol = Math.min(
            maxCols - 1,
            Math.ceil((this.scrollX + canvasWidth - this.config.rowHeaderWidth) / this.config.cellWidth) + 1
        );
    };
    ...
    TableRenderer.prototype.drawCells = function(tableData) {
        ...
        // 设置全局裁剪区域：确保内容不会绘制到行号头部区域
        this.ctx.save();
        this.ctx.beginPath();
        this.ctx.rect(
            this.config.rowHeaderWidth, 
            this.config.headerHeight, 
            this.canvas.clientWidth - this.config.rowHeaderWidth, 
            this.canvas.clientHeight - this.config.headerHeight
        );
        this.ctx.clip();
        ...
        for (var row = this.viewport.startRow; row <= this.viewport.endRow; row++) {
            for (var col = this.viewport.startCol; col <= this.viewport.endCol; col++) {
                ...
                // 跳过完全不可见的单元格（修复：允许部分被行号头部遮挡的单元格绘制）
     ...
     * 绘制新的交互式滚动条
     */
    TableRenderer.prototype.drawNewScrollbars = function() {
        // 使用注入的滚动条组件
        if (this.scrollbarWidget) {
            this.scrollbarWidget.render(this.ctx);
        }
    };
     ...
     * 立即渲染（绕过节流限制）
     */
    TableRenderer.prototype.renderImmediate = function(tableData, selectedCell) {
        if (this._originalRender) {
            this._originalRender(tableData, selectedCell);
        }
    };
